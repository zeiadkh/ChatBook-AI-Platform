...
import { resetTemp, confirmationTemp } from "../../utils/htmlTemp.js";
...
export const register = async (req, res, next) =>{
    ...
    const user = await User.create({userName, email, password, activationCode, role})
    ...
    console.log(email)
    !await sendEmail({to: email?email: "", subject: "Confrimation Your Email", temp: confirmationTemp(confirmationLink)})?
    next(new Error("Couldn't Verify your email")): ''
    return res.json({succes: true, message: "Account Created Successfully, Please Check your email"})
}
